...
def add_file(log_path):
    default_path = os.path.join(os.path.expanduser("~"), "C:\\DIST\\CONFIG\\DASEUL\\CABLE_LOSS\\")
...
def Check_mipi_R(text_area, mipi_data, combo4):
...
def Check_mipi_W(text_area, mipi_data, combo4):
    text_area.delete("1.0", tk.END)
    Comport = combo4.get()
    ch = mipi_data[0].get()
    usid = mipi_data[1].get()
...
def Read_mipi(dut, mipi_data, addr, text_area):
    ch = mipi_data[0].get()
    usid = mipi_data[1].get()
    at_resp = dut.at_write(f"AT+MIPIREAD = {ch},{usid},{addr}")
...
def Write_mipi(dut, mipi_data, addr, data, text_area):
    ch = mipi_data[0].get()
    usid = mipi_data[1].get()
    at_resp = dut.at_write(f"AT+MIPIWRITE = {ch},{usid},{addr},{data}")
...
def loss_table(Selected_path, Equip, Callbox):
    with open(Selected_path, "r", encoding="utf-8") as loss_file:
        ...
        if ext == ".txt":
            ...
            df = pd.read_csv(loss_file, sep="=", names=["index", "value"], skiprows=[0, 1, 2, 3, 4])
        else:
            df = pd.read_csv(
                loss_file,
                sep="=",
    ...
    if (Equipment == "CMW") & (Equip == "TCPIP0::127.0.0.1"):
        ...
        df_freq = df.loc[df["g"] == 0, "value"].astype(int).astype(str) + "e6"  # Frequency 정수로 변환 후 문자열로 변환 & e6 추가
        df_loss = df.loc[df["g"] == 1, "value"].astype(float).abs()  # loss value
        df["value"] = df_freq.combine_first(df_loss)  # 2개 Series freq 기준으로 병합
        loss_list = list(df["value"])  # value 값 loss_list에 list로 저장
        loss = return_print(*loss_list)
...
def OCP_Check(Sys_vcc, Pa_vcc, text_area):
    ...
    for Psupply in E3632a:
        ...
        if (Outp_stat == "0") or (int(float(Psupply.query("MEAS:CURR?")) * 1000) <= 5):
            Psupply_vcc_on_off(Psupply, Volt)
            if Psupply == Sys_vcc:  # System 전원을 off 했을때만 P_recycle True 설정
                P_recycle = True
...
def Equipment_scan():
    ...
    PS_list = []
    CP_list = []
    # dictionary로 key, value mapping 해서 gpib 넘버와 장비명 짝 맞춤 필요
    for r in Resource:
        if r.startswith("ASRL"):
            ...
            Equipment = rm.open_resource(r)
            Idn_Equip = Equipment.query("*IDN?").split(",")

            if Idn_Equip[1] == "CMW":
                # Callbox list
                CB_list.append(r)

            elif Idn_Equip[1] == "E3632A":
                # Power Supply list
                PS_list.append(r)

    # CMW100
    instr_list = ["TCPIP0::127.0.0.1::Instrument::INSTR"]

    for i in range(len(instr_list)):
        try:
            # 1.2초 내에 응답이 없으면 Equipment에 [0,0] 입력
            instrument = rm.open_resource(instr_list[i], open_timeout=1000)
    ...
    if Equipment[1] == "CMW":
        CB_list.append("TCPIP0::127.0.0.1")

    return CB_list, PS_list, CP_list
...
def images2PdfFile(save_dir, filename):
    ...
    filelist = sorted([file for file in os.listdir(save_dir) if file.endswith(r".png")])
...
def Nonsig_lte_tx_measure_setting(dut, Callbox, Equip, band, rxfreq, txfreq, text_area):
    ...
    Callbox.write("CONF:LTE:MEAS:MEV:RES:ALL OFF, OFF, OFF, OFF, OFF, OFF, OFF, ON, OFF, ON, OFF, OFF, OFF, OFF")
    Check_OPC(Callbox)
...
def Nonsig_nr_tx_measure_setting(dut, Callbox, Equip, band, rxfreq, txfreq, text_area):
    ...
    Callbox.write("CONF:NRS:MEAS:MEV:RES:ALL OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF")
